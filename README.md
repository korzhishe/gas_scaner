...
Если в релевантном Telegram-сообщении есть ссылка на статью, импортёр открывает эту страницу, вытаскивает заголовок, дату публикации, виды топлива, количество АЗС и короткие адресные подсказки. Такой материал сохраняется отдельным сигналом со ссылкой на статью, а исходный Telegram-пост остаётся отдельным источником.

Основной экран сайта работает в кризисном режиме: на карте и в списке показываются только АЗС, у которых есть свежий привязанный сигнал `fuel_available` или `no_fuel`. Плавающие общегородские новости остаются в ленте сигналов, но не добавляют станцию на карту без точной привязки.
...
- `SIGNAL_PARSE_LINKS=0` - отключить переходы по внешним ссылкам из Telegram.
- `SIGNAL_ARTICLE_MAX_PER_RUN=16` - максимум статей, которые импортёр попробует разобрать за один запуск.
